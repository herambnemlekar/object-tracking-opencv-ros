...
class OtpGenerator:
	...
	#Publishing the estimated goal position and total trajectory time
        ...
        self.baxter = np.matrix([[-0.2,0.15,0],[-0.2,-0.35,0],[0.2,0.15,0],[0.2,-0.35,0]])	#Baxter arm length 
	...
	if self.w>0:
		...
		if (self.e_old - self.e_new) > 0.001:	#Arbitrary threshold for detecting movement
			...
			self.w = self.w - (dt/self.t_goal)	#Calculating weights
			print("w", self.w)
		
			#Weighted average to find the goal
			self.P_goal = (self.w*self.otp_s) + ((1-self.w)*self.P_obj)
		
			V = np.linalg.norm((self.P_new - self.P_old)/dt)	#Velocity 
		
			self.t_goal = np.linalg.norm(self.P_goal - self.P_obj)/V 	#Estimating total time of the trajectory
